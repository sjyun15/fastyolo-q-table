...
class Environment:
    def __init__(self, frag=50, train=True):
        ...
        self.runtime = np.array([0, 0, 0, 0], dtype=int)
    ...
    def get_state(self, action=0): ## state, boxes, answer
        ...
        if len(image) >= 5:
            ...
            if len(image) == 0:
                ...
                avg /= len(image)
        if avg - 0.5 > 0:
            state = (int((avg - 0.5)/self.offset), np.sum(self.runtime))
        else:
            state = (0, np.sum(self.runtime))

        ## for next state
        if action:
            self.runtime[image_num%4] = 1
        else:
            self.runtime[image_num%4] = 0
    ...
    def step(self, state_list, action):
       ...
       ## get reward
        ...
        if isinstance(next_state_list[0], tuple):
